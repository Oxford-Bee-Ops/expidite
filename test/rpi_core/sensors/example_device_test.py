...
from expidite_rpi.example.my_sensor_example import EXAMPLE_FILE_DS_TYPE_ID, EXAMPLE_LOG_DS_TYPE_ID
...
class Test_example_device:
    ...
    def test_example_device(self):
        ...
        with RpiEmulator.get_instance() as th:
            ...
            sc.start()
            while not th.recordings_cap_hit(type_id=EXAMPLE_FILE_DS_TYPE_ID):
                sleep(1)
            ...
            d_count = (grouped_df.loc[grouped_df["observed_type_id"] == 
                                      EXAMPLE_DF_DS_TYPE_ID, "count"].values[0])
            f_count = (grouped_df.loc[grouped_df["observed_type_id"] == 
                                      EXAMPLE_FILE_DS_TYPE_ID, "count"].values[0])
            l_count = (grouped_df.loc[grouped_df["observed_type_id"] == 
                                      EXAMPLE_LOG_DS_TYPE_ID, "count"].values[0])
