...
# (max_recording_timer, default 180s) plus the journal flush; the upload queue itself is spilled to the disk
# spool at connector shutdown with no network I/O, so it adds almost nothing.
_REBOOT_FLUSH_TIMEOUT_SECONDS = 240.0

# Set the moment a reboot is first requested so repeated triggers become no-ops. Recovery callers fire on a
# poll (DeviceHealth checks memory every cycle; a memory-exhausted device stays over threshold), and a managed
# reboot collects a DiagnosticsBundle *before* it touches STOP_EXPIDITE_FLAG - a window in which the caller
# can fire again. Without this guard each repeat spawns another non-daemon reboot thread and another
# diagnostics collection on an already-OOM device, worsening the very condition we are recovering from. The
# flag is process-local: the two entry points run in different processes (RpiCore vs the management service)
# and never contend across them.
...
# Managed reboot
#
# All deliberate reboots must flush queued sensor data before the device goes down: data awaiting upload lives
# in RAM (the async upload queue, and /expidite which is a tmpfs on SD-card devices), so a bare `sudo reboot`
# would lose all of it. A clean shutdown stops sensors, flushes journals, and spills unsent uploads to the
# persistent disk spool, from which they are uploaded after the reboot.
#
# There are two entry points depending on where the caller runs:
#
# - stop_service_and_reboot(): for callers OUTSIDE the RpiCore process (BCLI, the management/IoT Hub service).
#   It runs `sudo systemctl stop expidite.service` - the same graceful stop the installer uses - which lets
#   systemd SIGTERM the whole service cgroup (tearing down in-flight work such as an rpicam-vid recording
#   promptly) and blocks until the service has fully exited. This is fast and is the preferred path.
#
# - request_managed_reboot(): for callers INSIDE the RpiCore process (device health / device manager fault
#   recovery). Such a caller cannot `systemctl stop` its own service without deadlocking (the stop  would wait
#   for the very process that is running the stop command to exit), so instead it sets STOP_EXPIDITE_FLAG -
#   which the EdgeOrchestrator main loop polls - and waits (bounded) for the run to finish before rebooting.
##############################################################################################################
