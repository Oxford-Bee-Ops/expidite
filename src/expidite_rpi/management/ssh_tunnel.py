...
The portal triggers a tunnel via the ``open_ssh_tunnel`` IoT Hub direct method. The device then dials
*outbound* to the portal over a WebSocket (the device never accepts inbound connections), and bridges that
WebSocket to its local sshd. See ``docs/ssh-tunnel-protocol.md`` for the wire contract.

The byte pump is deliberately decoupled from the real network libraries (the WebSocket connect and the
local-socket connect are injected) so the bridge logic can be unit-tested with mocks.
"""
...
# The local sshd port. Fixed (not configurable, not taken from the payload) so a tunnel can only ever reach
# sshd, never an arbitrary loopback service.
...
def _default_sock_connect(host: str, port: int) -> SocketLike:
    ...
    # create_connection leaves its connect timeout on the socket; clear it so the bidirectional pump uses
    # blocking reads. Otherwise sock.recv() raises socket.timeout after an idle gap (an interactive shell with
    # no output) and the tunnel is torn down mid-session.
...
class SshTunnelManager:
    ...
    A single instance is owned by the IoTHubClient. Validation is synchronous (so the direct-method ack can
    report acceptance), while the dial-out and byte pump run on a daemon thread.
    """
    ...
    def _run_session(self, payload: dict[str, Any]) -> None:
        ...
        # The device always bridges to its local sshd. The port is fixed (not taken from the payload) so a
        # tunnel can never be pointed at an arbitrary loopback service.
