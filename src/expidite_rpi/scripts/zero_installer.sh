...
# only for the specific steps that need it. Running the whole script as root (e.g. `sudo zero_installer.sh`)
...
check_prerequisites() {
    ...
    # Zero installer supports both 32-bit and 64-bit OS images.
    if [ "$(getconf LONG_BIT)" == "64" ] || [ "$(uname -m)" == "aarch64" ]; then
        echo "64-bit OS detected"
    else
        echo "32-bit OS detected (armv7/armhf)."
        echo "Proceeding with install for Pi Zero profile."
    fi
...
install_expidite() {
    ...
    if [[ "$EXP_REMOTE_HASH" != "$EXP_LOCAL_HASH" || "$new_install" == "yes" ]]; then
        ...
        if pip_install "${PIP_HEAL_ARGS[@]}"                 "git+https://github.com/oxford-bee-ops/expidite.git@$expidite_git_branch"; then
...
set_log_storage_volatile() {
    ...
    # Keep journald smaller on low-memory devices.
    total_mem_mb=$(get_total_mem_mb)
    if [ "$total_mem_mb" -le 700 ]; then
        journal_size="20M"
    elif [ "$total_mem_mb" -le 1500 ]; then
        journal_size="30M"
    else
        journal_size="50M"
    fi
...
create_mount() {
    ...
    if [[ "$root_dev" == /dev/sd* || "$root_dev" == /dev/nvme* ]]; then
        ...
        if [ "$total_mem_mb" -le 700 ]; then
            ...
            mount_size="256M"
        elif [ "$total_mem_mb" -le 3000 ]; then
            mount_size="512M"
        else
            mount_size="1200M"
        fi
        echo "Detected RAM: ${total_mem_mb}MB. Using tmpfs size $mount_size."
...
reboot_if_required() {
    ...
    if reboot_is_pending; then
        ...
        "$BOOT_HISTORY_PYTHON" "$BOOT_HISTORY_SCRIPT" record-request \
            "zero_installer requested reboot #$reboot_count"
        if ! sudo reboot; then
